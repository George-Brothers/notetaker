...
- `cargo check --target aarch64-apple-darwin` — compiles `cidre`, the Apple
  frameworks crate. 3.5 s.

So the platform layer goes in its own crate whose deps are pure Rust
(`wasapi`, `windows`, `cidre`, `cpal`), keeping the heavy C++ deps
(`whisper-rs`, `sherpa-rs`, bundled `rusqlite`) out of the cross-check path.
That buys a fast local loop that catches the dominant failure mode of
...
5. **macOS `AudioSource` + `SystemProbe`.** Mic via `cpal`, system audio via
   ScreenCaptureKit through `cidre`. Replace the `ioreg`/`pmset` shell-out
   `MacProbe` with a `cidre` probe, which retires "verified vs assumed" risk #1
   in `docs/MAP.md`; keep the shell-out parsers as a documented fallback.
6. **Cross-platform Tauri shell.** The app crate is still the generated
