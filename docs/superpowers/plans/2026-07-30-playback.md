...
- `audio_tracks` counted a 0-byte file as a track, so every player on his
  machine defaulted to the empty `audio-system.flac` and played silence. That
  is the honest half of the 0-byte item in §5; the stray `audio-mic.wav`
  beside the `.flac` is still open.
