...
`audio_tracks` filters on `.exists()`, and his three recordings each carry a
0-byte `audio-system.flac` — nothing was playing through his speakers, so
WASAPI loopback correctly produced nothing. The field's own doc comment already
states the intent the code misses:
