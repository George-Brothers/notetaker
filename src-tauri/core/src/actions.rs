...
";
    ...
    fn a_two_word_name_is_an_owner() {
        let items = parse("- [ ] Mr. Brothers: approve the budget\n");
        // "Mr. Brothers" contains a period, so it is deliberately not claimed
        // as an owner rather than risking a wrong chip.
        assert_eq!(items[0].owner, None);

        let items = parse("- [ ] George Brothers: approve the budget\n");
        assert_eq!(items[0].owner.as_deref(), Some("George Brothers"));
    }
