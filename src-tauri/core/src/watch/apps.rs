...
//! [`BROWSER_APPS`].
//!
//! **Process names are macOS's**, since macOS is the target; where the Linux
//! name differs it is listed alongside, because the fake-driven tests and all
//! dev-machine runs happen on Linux. Linux truncates `comm` to 15 bytes, so a
//! longer Linux name may not match there — harmless, as Linux is not the
//! shipping platform.
...
pub const KNOWN_APPS: &[MeetingApp] = &[
    MeetingApp {
        ...
        // `zoom`. `ZoomPhone` is deliberately absent — a phone call is not a
        // meeting we have both sides of.
    ...
    MeetingApp {
        ...
        process_names: &["MSTeams", "Teams", "teams-for-linux"],
    },
    MeetingApp {
        id: "slack",
        display_name: "Slack",
    ...
    MeetingApp {
        ...
        // standalone meetings client some enterprises still pin to.
        process_names: &["Webex", "Cisco Webex Meetings"],
    },
    MeetingApp {
        id: "discord",
        display_name: "Discord",
        // Capitalized on both macOS and the Linux AppImage.
